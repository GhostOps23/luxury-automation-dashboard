    ...
    <div className="w-64 bg-dark-900 border-r border-dark-700 flex flex-col">
      ...
      <div className="p-6 border-b border-dark-700">
        <div className="flex items-center space-x-3">
          <div className="w-8 h-8 bg-gradient-to-r from-luxury-600 to-luxury-400 rounded-lg flex items-center justify-center">
      ...
      <nav className="flex-1 px-4 py-6 space-y-2">
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              <item.icon className={`w-5 h-5 ${isActive ? 'text-luxury-400' : 'text-gray-400'}`} />
              <span className={`font-medium ${isActive ? 'text-luxury-400' : 'text-gray-300'}`}>
                {item.name}
              </span>
            </Link>
          )
